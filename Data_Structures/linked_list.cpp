...
public:
    ...
    Node *firstNode()
    { // Baş düğümün gösterdiğini düğümü döndürür
        return root->next;
    }
    void add(const int &newData)
    { // int değerini listeye ekler
        Node *tmp = root;
        while (tmp->next != NULL)
        {
            tmp = tmp->next;
    ...
    { // Diziyi alır ve listeye ekler
        for (int i = 0; i < len; i++)
        {
            this->add(array[i]);
        }
    }
    const int &get(int location)
    { // Belirtilen konumdaki düğümün verisini döndürür
        int index = 0;
        Node *tmp = root;
